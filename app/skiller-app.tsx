...
import { ArrowLeft, ArrowRight, BrainCircuit, CalendarClock, Check, ClipboardList, Dumbbell, Home, ShieldAlert, Sparkles, Target, UserRound, Zap } from "lucide-react";
...
type Recommendation = { situationId: string; safetyStatus: "self-guided" | "escalate"; skill: SkillView | null; changePoint?: string; reason?: string };
...
export function SkillerApp({ initialData }: { initialData: DashboardData }) {
  ...
  async function askForRecommendation(event: React.FormEvent) {
    ...
    try { setRecommendation(await post({ action: "recommend", kind, description, firstSignal, actionUrge, desiredDirection, importantGoal, intensity, risk })); }
  ...
  async function saveOutcome() {
    ...
    try { setData(await post({ action: "complete", attemptId: session.attemptId, ...outcome })); setSession(null); setTab("today"); flash("Результат сохранён. Позже проверим, сохранился ли эффект"); }
  ...
  async function saveOnboarding() {
    ...
    } catch (error) { flash(error instanceof Error ? error.message : "Не удалось сохранить ответы"); }
    finally { setBusy(false); }
  }
  ...
  async function saveDelayedOutcome() {
    ...
    } catch (error) { flash(error instanceof Error ? error.message : "Не удалось сохранить проверку"); }
    finally { setBusy(false); }
  }
    ...
    <Tabs value={tab} onValueChange={setTab} className="min-h-screen bg-[#f4f7fb] text-[#132348]">
      ...
      <Dialog open={checkInOpen && Boolean(data.pendingCheckIn)} onOpenChange={setCheckInOpen}>
        <DialogContent className="max-h-[92vh] overflow-y-auto rounded-[26px] border-[#dce4f0] p-5 sm:max-w-xl sm:p-7">
          <DialogHeader>
            <p className="eyebrow">Проверка спустя время</p>
            <DialogTitle className="text-2xl font-black text-[#132348]">Что осталось после навыка?</DialogTitle>
            <DialogDescription className="text-base leading-6">Сразу могло стать легче. Сейчас важнее понять, помогло ли действие приблизиться к цели.</DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-5 pt-2">
            <Rating label="Насколько результат сохранился?" value={delayedOutcome.helpfulness} min={0} max={10} left="Не сохранился" right="Сохранился" onChange={(value)=>setDelayedOutcome((current)=>({...current,helpfulness:value}))}/>
            <Rating label="Насколько это приблизило к цели?" value={delayedOutcome.goalProgress} min={0} max={10} left="Не приблизило" right="Приблизило" onChange={(value)=>setDelayedOutcome((current)=>({...current,goalProgress:value}))}/>
            <label className="flex items-start gap-3 rounded-2xl border border-[#dce4f0] bg-[#f8faff] p-4"><Checkbox checked={delayedOutcome.avoidance} onCheckedChange={(checked)=>setDelayedOutcome((current)=>({...current,avoidance:Boolean(checked)}))}/><span><strong className="block">Навык помог отложить важное действие</strong><small className="mt-1 block leading-5 text-[#718099]">Это снизит уверенность системы, даже если сразу стало легче.</small></span></label>
            <Textarea value={delayedOutcome.note} onChange={(event)=>setDelayedOutcome((current)=>({...current,note:event.target.value}))} placeholder="Что произошло после практики? Необязательно" className="min-h-20 rounded-xl"/>
            <Button disabled={busy} onClick={saveDelayedOutcome} className="h-13 rounded-xl bg-[#2868f5] text-base font-bold">{busy ? "Сохраняем…" : "Обновить личную карту"}</Button>
          </div>
        </DialogContent>
      </Dialog>
...
function OnboardingDialog({ open, step, draft, busy, onStep, onChange, onSave }: { open: boolean; step: number; draft: OnboardingDraft; busy: boolean; onStep: (step:number)=>void; onChange:(patch:Partial<OnboardingDraft>)=>void; onSave:()=>void }) {
  const titles = ["С чего начнём?", "Как выглядит нужное изменение?", "Как удобнее практиковаться?", "Кто будет рядом?"];
  const descriptions = [
    "Выберите одну область для первого маршрута. Это не диагноз — фокус можно будет изменить.",
    "Один наблюдаемый результат даст системе ориентир для проверки навыков.",
    "SKILLER будет учитывать нагрузку, но не станет прятать важные шаги за комфортом.",
    "Самостоятельная практика и работа с психологом могут дополнять друг друга.",
  ];
  const canContinue = step === 0 ? Boolean(draft.focus) : step === 1 ? draft.goal.trim().length >= 5 : step === 3 ? draft.safetyAcknowledged : true;
  return <Dialog open={open} onOpenChange={()=>{}}><DialogContent showCloseButton={false} onEscapeKeyDown={(event)=>event.preventDefault()} onPointerDownOutside={(event)=>event.preventDefault()} className="max-h-[94vh] overflow-y-auto rounded-[28px] border-[#dce4f0] p-0 sm:max-w-[640px]">
    <div className="bg-[#132348] px-5 py-5 text-white sm:px-7"><div className="flex items-center justify-between"><span className="font-black tracking-[.12em]">SKILLER</span><span className="text-sm text-white/65">{step+1} из 4</span></div><div className="mt-4 grid grid-cols-4 gap-2" aria-label={`Шаг ${step+1} из 4`}>{[0,1,2,3].map((item)=><span key={item} className={`h-1.5 rounded-full ${item<=step?'bg-[#62ddc3]':'bg-white/15'}`}/>)}</div></div>
    <div className="p-5 sm:p-7"><DialogHeader><p className="eyebrow">Личный маршрут</p><DialogTitle className="mt-1 text-2xl font-black tracking-tight text-[#132348] sm:text-3xl">{titles[step]}</DialogTitle><DialogDescription className="text-base leading-7 text-[#718099]">{descriptions[step]}</DialogDescription></DialogHeader>
      <div className="mt-6">
        {step===0&&<div className="grid gap-2 sm:grid-cols-2">{[["start","Начинать важные дела","Прокрастинация и замирание"],["emotions","Выдерживать сильные эмоции","Тревога, стыд, злость"],["relationships","Строить отношения и границы","Просьбы, конфликты, близость"],["impulses","Не действовать на автомате","Импульсивные реакции"],["loneliness","Создавать и сохранять связи","Дружба и одиночество"]].map(([value,title,copy])=><button key={value} onClick={()=>onChange({focus:value})} className={`choice min-h-20 text-left ${draft.focus===value?'choice-active':''}`}><strong className="block">{title}</strong><small className="mt-1 block font-normal leading-5 text-[#718099]">{copy}</small></button>)}</div>}
        {step===1&&<><label className="field-title" htmlFor="onboarding-goal">Через несколько недель что станет заметно в жизни?</label><Textarea id="onboarding-goal" autoFocus value={draft.goal} onChange={(event)=>onChange({goal:event.target.value})} className="min-h-32 rounded-xl bg-[#f8faff] text-base" placeholder="Например: дважды в неделю первым писать знакомым и предлагать встретиться"/><p className="mt-3 text-sm leading-6 text-[#718099]">Лучше описать действие, а не состояние вроде «стать уверенным».</p></>}
        {step===2&&<div className="grid gap-2">{[["short","Коротко и часто","2–3 минуты, один конкретный шаг"],["guided","Подробно и с объяснением","Больше контекста перед практикой"],["mixed","По ситуации","Коротко в моменте, подробнее в тренировке"]].map(([value,title,copy])=><button key={value} onClick={()=>onChange({practiceStyle:value})} className={`choice min-h-20 text-left ${draft.practiceStyle===value?'choice-active':''}`}><strong className="block">{title}</strong><small className="mt-1 block font-normal leading-5 text-[#718099]">{copy}</small></button>)}</div>}
        {step===3&&<div className="grid gap-4"><div className="grid gap-2">{[["solo","Пока самостоятельно","Психолога можно подключить позже"],["own_psychologist","Есть свой психолог","Доступ появится только после вашего согласия"],["specialist_later","Хочу разбор со специалистом","Когда накопится личная карта"]].map(([value,title,copy])=><button key={value} onClick={()=>onChange({supportMode:value})} className={`choice min-h-20 text-left ${draft.supportMode===value?'choice-active':''}`}><strong className="block">{title}</strong><small className="mt-1 block font-normal leading-5 text-[#718099]">{copy}</small></button>)}</div><label className="flex items-start gap-3 rounded-2xl border border-[#efc6cd] bg-[#fff6f7] p-4"><Checkbox checked={draft.safetyAcknowledged} onCheckedChange={(checked)=>onChange({safetyAcknowledged:Boolean(checked)})}/><span><strong className="block">Я понимаю границы самостоятельной практики</strong><small className="mt-1 block leading-5 text-[#71535a]">При риске причинить вред себе или другому SKILLER остановит подбор и предложит обратиться за живой помощью.</small></span></label></div>}
      </div>
      <div className="mt-7 flex items-center justify-between gap-3">{step>0?<Button variant="ghost" onClick={()=>onStep(step-1)} className="h-12 rounded-xl"><ArrowLeft/> Назад</Button>:<span/>}<Button disabled={!canContinue||busy} onClick={()=>step<3?onStep(step+1):onSave()} className="h-12 rounded-xl bg-[#2868f5] px-5 font-bold">{step<3?<>Продолжить <ArrowRight/></>:busy?"Сохраняем…":"Собрать первый маршрут"}</Button></div>
      <p className="mt-5 text-center text-xs leading-5 text-[#8793a7]">Ответы используются только для подбора и проверки навыков. Это не медицинская диагностика.</p>
    </div>
...
function RecommendationCard({ recommendation, busy, onStart }: { recommendation: Recommendation; busy:boolean; onStart:(skill:SkillView,mode:"help",situationId?:string)=>void }) { const skill = recommendation.skill!; return <section className="surface mt-4 p-5 md:p-7"><div className="flex items-center justify-between gap-3"><span className="pill bg-[#e8efff] text-[#2868f5]">Подходит к ситуации</span><span className="text-sm text-[#718099]">{Math.ceil(skill.durationSeconds/60)} мин</span></div><p className="eyebrow mt-6">{skill.approach}</p><h2 className="mt-2 text-2xl font-black">{skill.title}</h2><p className="mt-3 leading-7 text-[#627089]">{skill.description}</p><div className="my-5 grid gap-3 rounded-2xl bg-[#f0f5ff] p-4"><div><strong>Точка изменения</strong><p className="mt-1 text-sm leading-6 text-[#51658e]">{recommendation.changePoint}</p></div><div className="border-t border-[#d8e3f8] pt-3"><strong>Почему этот навык</strong><p className="mt-1 text-sm leading-6 text-[#51658e]">{recommendation.reason ?? skill.why}</p></div></div><Button disabled={busy} onClick={()=>onStart(skill,"help",recommendation.situationId)} className="h-13 w-full rounded-xl bg-[#2868f5] text-base font-bold">Начать навык</Button></section>; }
